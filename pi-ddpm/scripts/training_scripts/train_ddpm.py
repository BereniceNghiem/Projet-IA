...
def train_pinn_ddpm(type='widefield'):
    ...
    w2s_test = np.load('/home/ids/bnghiem-23/Projet-IA-Telecom-Paris/pi-ddpm/data/w2s_test.npz')
    print('loading w2s_test')
    Yw2s_test = w2s_test['y']
    ...
    for step in range(train_steps):
        if step % ds_sample == 0:
            ...
            if type == 'widefield':
                KTrain = np.expand_dims(w2s_test['widefield_kernel'], -1)
            else:
                KTrain = np.expand_dims(w2s_test['confocal_kernel'], -1)

            print('current learning rate: ' + str(t_model.optimizer.learning_rate.numpy()))
            avg_loss = 0
        ix = np.random.randint(0, Yw2s_test.shape[0], batch_size)
        kx = np.random.randint(0, KTrain.shape[0], batch_size)
        fy_b = tf.image.resize(Yw2s_test[ix], [patch_size, patch_size]).numpy()
        fy = norm_01(fy_b) * 2 - 1
        ...
        if step % print_freq == 0:
            ...
            plt.imsave('/home/ids/bnghiem-23/Projet-IA-Telecom-Paris/pi-ddpm/imgs_output/img_sr' + str(step) + '.png',
                       np.uint8(np.clip(c_imgs_result, -1, 1) * 127.5 + 127.5))

        if step % 20000 == 0:
            t_model.save_weights(
                '/home/ids/bnghiem-23/Projet-IA-Telecom-Paris/pi-ddpm/models_weights/df_ddpm/model' + str(step) + '.h5')

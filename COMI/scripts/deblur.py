...
def load_confocal(input_shape=None, set=None, z_depth=None):
    ...
    dir = '/home/ids/bnghiem-23/Projet-IA-Telecom-Paris/Dataset' + set
    lr_lq_set = []
    hr_lq_set = []
    lr_hq_set = []
    hr_hq_set = []
    ...
    lr_hq_set = []
    hr_hq_set = []
    for _, _, files in os.walk(dir+'/'+z_depth):  #files ne récupèrent que les dossiers du fichier principal
        for file in files:
            if int(file.split('_')[-1].split('.')[0]) >= len(files) * 0.8:
                img_lq = cv2.imread(dir+'/'+z_depth + '/' + file)
                img = cv2.resize(img_lq, (input_shape[0], input_shape[1]))
